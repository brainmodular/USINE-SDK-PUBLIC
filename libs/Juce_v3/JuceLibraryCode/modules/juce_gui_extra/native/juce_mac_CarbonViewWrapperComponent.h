...
public:
    ...
    OSStatus carbonEventHandler (EventHandlerCallRef nextHandlerRef, EventRef event)
	{
		...
		if (GetEventClass(event) == kEventClassKeyboard)
		{
			...
			{
				OSStatus result = CallNextEventHandler(nextHandlerRef, event);
				...
				if (result == eventNotHandledErr)
				{
					...
					GetEventParameter(event, kEventParamKeyMacCharCodes, typeChar, NULL, sizeof(juce_wchar), NULL, &textCharacter);
					String unmodified = String::charToString(textCharacter);

					int keyCode = (int)unmodified.toUTF8()[0];
					getPeer()->handleKeyUpOrDown(true);
					getPeer()->handleKeyPress(keyCode, textCharacter);
				}
			}
			return noErr;
			//return CallNextEventHandler(nextHandlerRef, event);
			...
			case kEventRawKeyModifiersChanged:
			{
				...
				if (result == eventNotHandledErr)
					getPeer()->handleModifierKeysChange();
			}
			return noErr;
			//return CallNextEventHandler(nextHandlerRef, event);

			// SENSOMUSIC END return noErr;
			}
		}

		// SENSOMUSIC CHANGE END
		// ------------------------------------------	
