...
UserModuleBase::~UserModuleBase()
{
    if (m_job_thread != nullptr) {
        sdkThreadDestroy(m_job_thread);
    }
}
...
void UserModuleBase::CallBack(TUsineMessage* Message)
{
    ...
    {
        std::string os;
        os += "CallBack, CallbackID=";
        os += std::to_string(Message->wParam);
//       os += std::format("0x{:0{}X}", Message->wParam, 8);
        sdkTraceErrorChar(os);
    }
}
...
void UserModuleBase::startJob()
{
    if (!m_taskRunning) {
        ...
        try {
            if (m_job_thread == nullptr) {
                m_job_thread = sdkThreadCreate("thread job", &UserModuleBase::processThreadJob, tpMedium, UINT32_MAX);
            }
            sdkThreadRestart(m_job_thread);
        }
        catch (...) {
            m_taskRunning = false;
        }
...
void UserModuleBase::processThreadJob(void* pModule, TThreadPtr pThread)
{
    UserModuleBase* module = static_cast<UserModuleBase*>(pModule);
    try {
        module->onJobBegin();
        module->onJobProcess();
    }
    catch (...) {
        module->notifyUsineOnJobEnd();
    }
    module->notifyUsineOnJobEnd();
}

void UserModuleBase::notifyUsineOnJobEnd() const
{
    sdkNotifyUsine(NOTIFY_TARGET_USER_MODULE, NOTIFY_MSG_USINE_CALLBACK,
        CALLBACK_JOB_END, MSG_CHANGE, TNotifyUsineMode::numRealTime, 25
    );
}

void UserModuleBase::terminateJob()
{
    try {
        onJobEnd();
    }
...
void Create(void*& pModule, AnsiCharPtr optionalString, LongBool Flag, TMasterInfo* pMasterInfo, AnsiCharPtr optionalContent)
{
    ...
    // in case of a standard exception, we log it
    catch (std::exception& e)
    {
        pModule = nullptr;
        sdkTraceErrorChar(std::string(UserModuleBase::MODULE_NAME) + "/n Error upon creating module./n" + e.what());
#ifdef _WIN64
#pragma warning(suppress: 4297)
    ...
    catch (...)
    {
        ...
        pModule = nullptr;
        sdkTraceErrorChar(std::string(UserModuleBase::MODULE_NAME) + ": Error upon creating module.");
...
int GetChunkLen(void* pModule, LongBool Preset)
{
    ...
    return userModule->onGetStateChunkLen(Preset);
}
...
void GetChunk(void* pModule, void* chunk, LongBool Preset)
{
    ...
    userModule->onGetStateChunk(chunk, Preset);
}
...
void SetChunk(void* pModule, const void* chunk, LongBool Preset, int sizeInBytes)
{
    ...
    userModule->onSetStateChunk(chunk, sizeInBytes, Preset);
}
